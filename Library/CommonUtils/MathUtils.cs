// from https://github.com/perivar/CommonUtils/blob/master/Library/Source/MathUtils.cs
...
namespace CommonUtils
{
    ...
    public static class MathUtils
    {
        ...
        #endregion static Log and Exp methods
        ...
        #endregion ConvertRangeAndMaintainRatio
        ...
        #endregion ConvertAndMaintainRatio and Scale methods
        ...
        #endregion Jagged Array init using Linq
        ...
        public static double[] ComputeLogAverages(double[] spectrum, int logBins, int[] logFrequenciesIndex)
        {
        ...
        public static float[] ExtractLogBins(double[] complexSignal, int logBins, int[] logFrequenciesIndex)
        {
            var sumFreq = new float[logBins];
            ...
            for (int i = 0; i < logBins; i++)
            {
                ...
                sumFreq[i] = sumFreq[i] / (hiBound - lowBound + 1); // added + 1 at the end to ensure we get the edges
            }
            return sumFreq;
        }
        ...
        /// </summary>
        /// <param name="F">Frequency to be found in the spectrum vector [E.g. 300 Hz]</param>
        /// <param name="Fs">Frequency rate (e.g. Samplerate) at which the signal was processed [E.g. 44100 Hz]</param>
        /// <param name="N">Length of the fft spectrum [2048 elements generated by WDFT from which only 1024 are with the actual data]</param>
        ...
        /// </remarks>
        public static void GenerateLogFrequencies(double sampleRate, double minFreq, double maxFreq, int logBins, int fftSize, double logarithmicBase, out int[] indexes, out float[] frequencies)
        {
        ...
        /// </summary>
        /// <param name="k">index in the FFT spectrum</param>
        /// <param name="Fs">Frequency rate (e.g. Samplerate) at which the signal was processed [E.g. 5512Hz]</param>
        /// <param name="N">Length of the fft spectrum [2048 elements generated by WDFT from which only 1024 are with the actual data]</param>
        ...
        /// * Signals sampled at Fs are only able to accurately represent frequencies up to,
        ///   but not including Fs/2 (Nyquist rate).
        ///   This is why I showed that the mapping from k to Hz is only good for half the output samples.
        ///   I will not go into what the second half represents
        ///   (it will actually be a mirror image of the first half for a real input signal)
        /// * The output of the DFT/FFT is complex. You most likely want to take the magnitude of this.
        /// * If you need to compute even a few DFT outputs, it may be better to just use the FFT function available
        ///   and get all the output samples instead of calculating just the output samples you need using the DFT. The reason is that most FFT algorithms are heavily optimized so even though you may be theoretically doing less work, it may take longer than the FFT. You would probably just have to benchmark this to see which approach is better.
        /// </remarks>
        ...
        public static double IndexToFreq(int k, double Fs, int N)
        {
            return (double)k * (Fs / N);
        }

        #endregion Time, Index and Freq Conversion
        ...
        #endregion Float, Int And Double Conversions
        ...
        /// <returns></returns>
        public static string FormatNumber(UInt32 num)
        {
            if (num >= 100000000)
        ...
        #endregion Calculus (Hypot, Multiply etc)

        #region Correlation

        // from http://www.codeproject.com/Articles/49723/Linear-correlation-and-statistical-functions
        /// <summary>
        /// Calculate Pearson correlation coefficient
        /// </summary>
        /// <param name="x">1st vector</param>
        /// <param name="y">2nd vector</param>
                              ...
                              double[] y)
        {
            if (x == null || y == null)
            {
                throw new ArgumentNullException("Can not calculate correlation with null");
            }
            if (x.Length != y.Length)
            {
                throw new ArgumentException("Can not calculate correlation arrays of different size");
            }
            ...
            var r = sxy / (Math.Sqrt(sxx * syy) + TINY);
            //for a large n
        ...
        #endregion Extension methods (IndexesWhere, 2D Row and Column accessors, 2D Array Deep Copy and Array Splitting)
        ...
        #endregion Distribute items in columns evenly
        ...
        #endregion Find indices matching a search criteria
        ...
        #endregion Array methods (like array initialising ala Matlab), Zero padding methods and flatten
    }
}