// from https://github.com/perivar/FindSimilar/blob/master/DctMethods.cs
    ...
    public static double[][] dct2(double[][] A, double offset)
    {
        ...
        for (int k = 0; k < n; k++)
        {
            for (int l = 0; l < m; l++)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    var cin = (Math.PI * (2.0 * i + 1.0) * k) / (2.0 * n);
                    var cos = Math.Cos(cin);
                    //Console.WriteLine("cos {0} = {1}", cin, cos);
                    sum += (A[i][l] + offset) * cos;
                }
                alfa = k == 0 ? 1.0 / Math.Sqrt(n) : Math.Sqrt(2.0 / n);
                B[k][l] = alfa * sum;
            }
        ...
        for (int l = 0; l < m; l++)
        {
            for (int k = 0; k < n; k++)
            {
                ...
                for (int j = 0; j < m; j++)
                {
                    var cin = (Math.PI * (2.0 * j + 1.0) * l) / (2.0 * m);
                    var cos = Math.Cos(cin);
                    //Console.WriteLine("cos {0} = {1}", cin, cos);
                    sum += B[k][j] * cos;
                }
