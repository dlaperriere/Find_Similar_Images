...
{
[TestClass]
public class TestImageAverageHash
{
    ...
    public void AverageHash()
    {
        ...
        var hash_lena = Images.ImageAverageHash.AverageHash(lena_img);
        var hash_koala = Images.ImageAverageHash.AverageHash(koala_img);

        var hash_lena2 = Images.ImageAverageHash.AverageHash(lena);
        var hash_koala2 = Images.ImageAverageHash.AverageHash(koala);
        ...
        // similarity range
        Assert.IsTrue(Images.ImageAverageHash.Similarity(hash_lena, hash_koala) >= 0);
        Assert.IsTrue(Images.ImageAverageHash.Similarity(hash_lena, hash_koala) <= 100);

        // test same
        Assert.IsTrue(Images.ImageAverageHash.Compare(lena_img, lena_img) == 100);

        // test different
        Assert.IsTrue(Images.ImageAverageHash.Compare(lena_img, koala_img) != 100);
        ...
        foreach (var file in files)
            {
                ...
                var similarity = Images.ImageAverageHash.Compare(test_image, image);
